...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="/export">
        <nantais>
            <xsl:apply-templates select="./acteurs/acteur[./etatCivil/infoNaissance/villeNais eq 'Nantes' and
                count(./mandats/mandat/infosQualite/codeQualite[text() eq 'Président']) ge 1
                ]">
                <xsl:sort select="./etatCivil/ident/nom" />
            </xsl:apply-templates>
        </nantais>
    </xsl:template>
    ...
    <xsl:template match="acteur">
        <xsl:element name="personne">
            ...
            <xsl:apply-templates select="./mandats/*[./infosQualite/codeQualite/text() eq 'Président']">
                <xsl:sort select="dateDebut/translate(text(), '-', '')" />
            </xsl:apply-templates>
        </xsl:element>
    </xsl:template>
